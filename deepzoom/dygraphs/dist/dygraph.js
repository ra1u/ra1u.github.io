...
DygraphInteraction.movePan = function (event, g, context) {
  ...
  var newposCallback = g.getFunctionOption('newposCallback');
  if (newposCallback) {
  ...
  if ("development" != 'production') {
    ...
    OPTIONS_REFERENCE = // <JSON>
    {
      ...
      "newposCallback": {
        ...
        "parameters": [["g", "the reference graph"]],
        "description": "called when nev widow is to be drawn. This eables us to push nwe data in graph"
      },
 ...
 */Dygraph.prototype.toDataXCoord = function(x){if(x === null){return null;}var area=this.plotter_.area;var r=this.xAxisRange();var xRange=[r[0].valueOf(),r[1].valueOf()];if(!this.attributes_.getForAxis("logscale",'x')){return xRange[0] + (x - area.x) / area.w * (xRange[1] - xRange[0]);}else {var pct=(x - area.x) / area.w;return utils.logRangeFraction(xRange[0],xRange[1],pct);}}; /**
 ...
 * @private
 */Dygraph.prototype.doAnimatedZoom = function(oldXRange,newXRange,oldYRanges,newYRanges,callback){var _this5=this;var steps=this.getBooleanOption("animatedZooms")?Dygraph.ANIMATION_STEPS:1;var windows=[];var valueRanges=[];var step,frac;if(oldXRange !== null && newXRange !== null){for(step = 1;step <= steps;step++) {frac = Dygraph.zoomAnimationFunction(step,steps);windows[step - 1] = [oldXRange[0] * (1 - frac) + frac * newXRange[0],oldXRange[1] * (1 - frac) + frac * newXRange[1]];}}if(oldYRanges !== null && newYRanges !== null){for(step = 1;step <= steps;step++) {frac = Dygraph.zoomAnimationFunction(step,steps);var thisRange=[];for(var j=0;j < this.axes_.length;j++) {thisRange.push([oldYRanges[j][0] * (1 - frac) + frac * newYRanges[j][0],oldYRanges[j][1] * (1 - frac) + frac * newYRanges[j][1]]);}valueRanges[step - 1] = thisRange;}}utils.repeatAndCleanup(function(step){if(valueRanges.length){for(var i=0;i < _this5.axes_.length;i++) {var w=valueRanges[step][i];_this5.axes_[i].valueRange = [w[0],w[1]];}}if(windows.length){_this5.dateWindow_ = windows[step];}var callDrawGraph=true;var newposCallback=_this5.getFunctionOption('newposCallback');if(newposCallback){callDrawGraph = newposCallback.call(_this5,_this5);}if(callDrawGraph){_this5.drawGraph_();}},steps,Dygraph.ANIMATION_DURATION / steps,callback);}; /**

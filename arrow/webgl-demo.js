...
function initBuffers(gl) {
  ...
  for (var j = 0; j < nedges; ++j) {
	...
	for(var z=0;z<2;++z){
	   for(var i=0;i<3;++i){  
		  colors = colors.concat(faceColors[z]);
       }
	}
	//side rectangle
	for(var i=0;i<4;++i){
       colors = colors.concat(faceColors[2]);
	}
  }
  ...
  const indices = [];
  side_indeces = [0,1,2,2,3,1];	
  for(var i=0;i<nedges;++i){
    var k = 10; // 3+3+4 verices per edge
	var i0 = i*k;
	for(var j=0; j<6; ++j){   
	  indices.push(i0 + j);
	}
	for(var j=0;j<6; ++j){
     indices.push(i0 + 6 + side_indeces[j]);
	}
  }
...
function drawScene(gl, programInfo, buffers, deltaTime) {
  ...
  var scaleZ = 2 + Math.sin(2*timeRunning);	
  //resize in z
